...
describe('insurance routing test', () => {
  ...
  it('can post new policy', async ()=> {
    ...
    const visitApproveObj={
      visitNumber:visitId,
      insuranceComp:insuranceCompSaved.id,
      status:'pinding',
    };
    const visitApprove=new db.VisitApprove(visitApproveObj);
    const visitApproveSaved=await visitApprove.save();
    VisitApproveId=visitApproveSaved.id;
  ...
  it('get All visit Pending Visits', async ()=> {
    ...
    let res=await mockRequest.get(`/insurance/visits/${VisitApproveId}`);
    //console.log(res.body)
