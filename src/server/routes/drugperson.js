  ...
  try {
    ...
    if(drug.status != 'nonpaid'){
      if(req.body.status == 'active'){
        ...
        const modified = await Drug.findByIdAndUpdate(id,{status:'active' , timeStart: moment().format()});
        console.log('active',req.body.status);
        res.status(204).json(JSON.stringify(modified));
      } else if(req.body.status == 'done'){
        const modified = await Drug.findByIdAndUpdate(id,{status:'done' , timeEnd: moment().format()});
        res.status(204).json(JSON.stringify(modified));
      }else if(req.body.status == 'deleted'){
        const modified = await Drug.findByIdAndUpdate(id,{status:'deleted' , timeDeleted: moment().format()});
        res.status(204).json(JSON.stringify(modified));
      }else{
...
async function getVisitDrugs(req, res) {
  ...
  try {
    ...
    //console.log(visitDrugs);
    if(visitDrugs){
      const drugs =await Promise.all(visitDrugs.map(async drug =>{
...
async function updateOneDrug(req, res) {
  ...
  try {
    ...
    const updated = await Procedure.findByIdAndUpdate({_id:drugId , type:'drug'}, req.body,{new:true});
